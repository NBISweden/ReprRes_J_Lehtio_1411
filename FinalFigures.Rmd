...
scatterplot2<-function(data,x,y, hadjust=-0.2){
  ret=ggplot(data,aes(x=data[,x], y=data[,y]))+
    ...
    theme(legend.justification=c(0,1),legend.position=c(0.1,0.9),legend.title=element_blank(), legend.key.size=unit(8, "pt"),legend.text=element_text(size=5))
  return(ret)
}
# Standard scatterplot of two variables
...
```{r loaddata, eval=TRUE, debug=TRUE}
# Create infile if needed
if(!file.exists(paste(in_dir,"tumourExpressionData", sep="/"))){
  source("formatData.R", chdir=TRUE, print.eval=T, echo=T)
}
```
...
```{r savecorr}
outfile=paste(out_dir,paste("Lehtio.all",method,"tumour_correlationdata.txt",sep="."),sep="/")
write.table(corr_tumour,file=outfile,sep="\t", row.names=FALSE)

outfile=paste(out_dir,paste("Lehtio.all",method,"gene_correlationdata.txt",sep="."),sep="/")
...
pdf(file=paste(out_dir,"Lehtio_i.pdf",sep="/"))
grid.arrange(p1,p2,blank, layout_matrix = lay, heights=unit(c(6,2.3),"cm"))
dev.off()
```
...
```{r,figii}
 ...
 p4=scatterplot2(sds,rvar,pvar,hadjust=-0.12)+
   coord_fixed()+
   scale_x_continuous(sub("sd","SD",rvar),limits=c(0,NA))+
   scale_y_continuous(sub("sd","SD",pvar),limits=c(0,3.5))+
...
pdf(file=paste(out_dir,"Lehtio_ii.pdf",sep="/"))
grid.arrange(pl[[1]]+ggtitle("A"),pl[[2]]+ggtitle("B"),pl[[3]]+ggtitle("C"),p4+ggtitle("D"),p5+ggtitle("E"),layout_matrix=lay)
dev.off()
```
...
```{r,figiiEsave}
  ...
  write.table(groupcorr[groupcorr$group!="All",],paste(out_dir,"Lehtio_iiE_quantilecorrelations.txt",sep="/"),sep="\t", row.names=FALSE)
```
...
pdf(file=paste(out_dir,"Lehtio_iii.pdf",sep="/"))
grid.arrange(pl[[4]]+ggtitle("A"),pl[[5]]+ggtitle("B"),pl[[6]]+ggtitle("C"),layout_matrix=lay,respect=TRUE)
dev.off()
...
for(p in c("Prior","Posterior")){
  ...
  for(meta in rownames(pmeta)){
    pmeta[meta,"meancorr"]=mean(corrxp$correlations[rownames(corrxp) %in% unlist(exprdata$metagene[[meta]])])
  }
...
pdf(file=paste(out_dir,"Lehtio_iv.pdf",sep="/"))
grid.arrange(pl[[1]]+ggtitle("A"),pl[[2]]+ggtitle("B"),pl[[3]]+ggtitle("C"),pl[[4]]+ggtitle("D"),layout_matrix=lay,respect=TRUE)
dev.off()
```
...
for(p in c("Prior", "Posterior")){
  ...
  WriteXLS(meta4output, ExcelFileName=paste(out_dir,paste("Lehtio_v_",p,".XLSX",sep=""),sep="/"),)
}
```
...
pdf(file=paste(out_dir,"Lehtio_vi.pdf",sep="/"))
metasatplot(corrxp,cvar,"correlations",meta)+
...
mymetas = c(1,2,3,4,5)
for(meta in mymetas){
  write(corrxp$gene[corrxp$meta==meta],file=paste(out_dir,paste("list4GO.metagroup",meta,".target.txt",sep=""),sep="/"),ncolumns=1)
  write(corrxp$gene[corrxp$meta!=meta],file=paste(out_dir,paste("list4GO.metagroup",meta,".background.txt",sep=""),sep="/"),ncolumns=1)
}
# make a list with all as well
write(corrxp$gene,file=paste(out_dir,"list4GO.all.txt",sep="/"),ncolumns=1)
# lapply(tl, write, paste(out_dir,"list4GO.txt",sep="/"), append=TRUE, ncolumns=1)
```
...
pdf(file=paste(out_dir,"Lehtio_viii.pdf",sep="/"))
grid.arrange(p1+ggtitle("A"),p2+ggtitle("B"),layout_matrix=lay,respect=TRUE)
dev.off()
```
